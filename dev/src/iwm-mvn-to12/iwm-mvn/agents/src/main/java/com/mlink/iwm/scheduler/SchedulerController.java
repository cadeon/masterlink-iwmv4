...
public class SchedulerController implements AssignAndSchedule {
	...
	// for debugging output files
	static int GENERATE_OUTPUT = 0; 
			...
			*/
					...
					while (ply < plyLimit) {
					    ...
					    while (it < numIterations) {
					        ...
					        for (Job j: sid.getJobs()) {
					        	for (AssetResource ar: j.getAssetResources()) {
			 		        		if (assetResourceAssigned(ar) // Optimization:  Cache a list of unassigned AssetResources
					        				&& hc.assetRemovableFromAR(j,ar)
	//				        				&& acceptRemoveMove(ar.getId(), utilityCalculator.getRemoveUtility(ar, assetResourceToAssetMap.get(ar.getId())))) {
			 		        				&& acceptRemoveMove(ar)) {
