...
public class JobTaskHelper {
	...
	public void setJobTasks(List<JobTask> ljt) {
		jobtasks = ljt;
	}
	...
	public List<TaskPresentation> autocomplete(Object suggest) {
		...
		ArrayList<TaskPresentation> result = new ArrayList<TaskPresentation>();
		for (Task t : getTasks()) {
			if (t.getDescription() != null
					&& t.getDescription().toLowerCase()
							.startsWith(pref.toLowerCase()) || "".equals(pref))
				result.add(new TaskPresentation(t));
		}
		return result;
	}
	...
	public List<NonAssetResource> getWorkItemParts() {
		return currentJobTask.getTaskInstance().getTask()
				.getNonAssetResources();
	}
}
