 ...
 * -- Hard Constraints
 * 
 * Three other classes in this package are InputProcessor (which is used by the test bed to convert a file input into a SchedulingInputData object) and
 * TabuSearch, which implements the interface TabuList and provides reusable functionality for short-term tabu search memory, and HardCodedSidValidator, 
 * which is used to implement the SidValidtor interface (validates input data).
 * 
 * Details:
 ...
 * 
 * 9. Helper routines.  These are small, generl purpose methods (some are syntactic sugar).
 * 
