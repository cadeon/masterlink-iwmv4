...
public class MaintenanceJobEditor implements Serializable {
	...
	public String deleteWorkItem(ActionEvent event) {
		...
		log.info("Marking job task "+ currentTask.getId() +" for deletion.");
	...
	private Set<Long> removeDeletedJobTasks() {
		...
		while (jtIterator.hasNext()) {		
			...
			if (jobTask.getId()!=null ) { 
				...
				try {
					// remove job task from task instance & delete task instance
					TaskInstance ti = jobTask.getTaskInstance();
					jobTask.setTaskInstance(null);
					taskInstanceHome.setId(ti.getId());
					taskInstanceHome.getInstance();
					ti.getJobTasks().clear();
					ti.setTask(null);
					log.info("Removing task instance "+ ti.getId());
					taskInstanceHome.remove();
	...
	private Set<Long> updateJob() {
		...
		while (pruneJts.hasNext()) {
			...
			if (jt.getId() == null) {
				...
				newJobTasks.add(jt);
			} else if (!deletedTaskList.contains(jt)) { 
				// retained job task, so retain asset resources
	...
	private void createAddedJobTask(JobTask jobTask) {
		...
		taskInstanceHome.persist(); // ironically, this also persists the job task & sets its id
		taskInstanceHome.clearInstance();
		...
		log.info("Added job task : "+ (jobTask.getId()!=null?jobTask.getId()+" - ":"")+jobTask.getDescription());
	}
		...
		if (!deleteArs.isEmpty()) {
			...
			while (jarIterator.hasNext()) {
				...
				if (deleteArs.contains(jar.getAssetTemplate().getId())) {
					...
					jar.getTask().getAssetResources().remove(jar);
					jar.getJob().getAssetResources().remove(jar);
					log.info("Removing asset resource "+ jar.getId());
	...
	public void cancel() {
		...
		if (job.getId()!=null) {
			jobHome.refresh(jobHome.find(job.getId()));
		}
	}
