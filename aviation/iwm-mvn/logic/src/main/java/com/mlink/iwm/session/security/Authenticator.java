...
public class Authenticator {
	...
	public boolean authenticate() {
		...
		log.info("Found user by username {0}....", credentials.getUsername());
		...
		log.info("Password entered by username {0}....matches the DB store",
		...
		currentUser = user;
		log.info("set currentUser to {0}: ", user.getUid());
		return true;
	...
	public void logout() {
		...
		log.info(">>>>>>>>> user {0} clicked on logout()", currentUser
						.getUid());
		...
		try {
			...
			entityManager.flush();
			log.info(">>>>>>>>>currentLoginActivity persisted for userUid {0}",
					currentLoginActivity.getUser().getUid());
			;

		} catch (Exception e) {
			log.info(">>>>>>>>>currentLoginActivity persistence for userUid {0} failed!",
							currentLoginActivity.getUser().getUid());
		}
		identity.logout();
	}
